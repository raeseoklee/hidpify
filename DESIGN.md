...
| R2 | After sleep/wake, a long-running daemon's WindowServer/graphics connection goes stale — a virtual display it then creates never registers in `CGGetOnlineDisplayList`, so HiDPI doesn't recover | **(confirmed, fixed v0.1.8)** | On `screensDidWake` the daemon exits and launchd (KeepAlive) restarts it with a fresh connection, which reliably re-applies (a fresh process always works). Deliberately no teardown on sleep — its reconfiguration otherwise bounces the just-slept panel back awake (v0.1.9) |
...
| R8 | **(confirmed, structural)** In mirror mode the physical panel stays lit (shows the desktop) while the system sleeps the displays — it mirrors the always-on software virtual display, so macOS treats the mirror set as active and won't sleep the panel (independent displays sleep normally) | Occurring | Not cleanly solvable without breaking the mirror during sleep, which fights `reapply` and risks wake bounces. Accepted as a virtual+mirror side effect (same class as R7 and the streaming ghost desktop); functionally harmless — HiDPI recovers on wake |
