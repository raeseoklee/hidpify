...
```sh
brew uninstall --cask raeseoklee/tap/hidpify         # 앱 제거 (데몬은 유지)
brew uninstall --zap --cask raeseoklee/tap/hidpify   # 데몬·설정·로그까지 제거
brew uninstall raeseoklee/tap/hidpify                # CLI/데몬 바이너리 제거
```

데몬은 (업데이트에 안 흔들리도록) 앱과 의도적으로 분리돼 있어, 일반 cask 삭제로는 데몬이 남습니다. 데몬까지 제거하려면 위의 `--zap`을 쓰거나 `hidpify uninstall-agent`를 실행하세요.
...
- **절전/복구**: 디스플레이가 깨어나면 HiDPI가 자동으로 다시 적용됩니다. 복구 시 데몬이 스스로 재시작하는데(장시간 떠 있던 프로세스는 절전을 거치면 WindowServer/그래픽 연결이 stale해져 이후 만든 가상 디스플레이가 등록이 안 되므로 fresh 프로세스가 필요), 가상 디스플레이를 다시 만들기까지 잠깐의 지연/깜빡임이 있습니다.
- **알려진 한계 — 절전 시 미러 대상이 꺼지지 않음(미러 모드)**: 시스템이 디스플레이를 재우는 동안, 미러링 대상 물리 디스플레이는 검게 꺼지지 않고 데스크탑을 계속 표시합니다. 항상 렌더링하는 가상 디스플레이를 미러링하기 때문에(가상은 소프트웨어 디스플레이라 잠들지 않음), macOS가 그 미러 세트를 활성으로 보고 물리 패널을 재우지 않습니다(독립 디스플레이는 정상 절전). 가상+미러 방식의 구조적 부작용으로, 기능에는 영향 없고(복구 시 HiDPI 정상) 미러를 끊지 않고는 깔끔히 없앨 수 없습니다.
